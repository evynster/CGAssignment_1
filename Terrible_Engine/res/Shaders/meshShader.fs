...
void main()
{
   ...
   vec3 nor = normalize(mat3(a_view) * Normal);
   vec3 p = vec3(a_view * vec4(viewPos, 1.0));//PROBLEM CHILD
   vec3 v = normalize(-p);
   ...
   vec3 lightResult = vec3(0.0,0.0,0.0);
        ...
        lightResult = lightResult + (vdn * rimToggle);
        //lightResult.rbg = vec3(smoothstep(0.6,1.0,vdn));
        //lightResult.a = 1.0;
